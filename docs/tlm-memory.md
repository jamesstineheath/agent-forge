...
- [2026-03-19] lib/atc.ts was decomposed into lib/atc/*.ts modules (dispatcher.ts, supervisor.ts, health-monitor.ts, events.ts, utils.ts, types.ts). However, lib/atc/dispatcher.ts has inherited the high-churn problem — modified in 7+ PRs on March 19 alone (PRs #315, #328, #340, #346, #352, #358, #363). lib/types.ts is also high-churn with 6 PRs modifying it on the same day. (high): Flag modifications to lib/atc/dispatcher.ts, lib/atc/supervisor.ts, lib/atc/health-monitor.ts, and lib/types.ts for extra scrutiny. Check for interface stability, backward compatibility, and concurrent modification conflicts. Consider whether dispatcher.ts itself needs further decomposition.
...
- [2026-03-19] .github/ workflow and action files (tlm-review.yml, tlm-feedback-compiler, action dist bundles) are modified frequently (7 PRs on March 19). Workflow changes have a historical 100% failure rate from the March 15 batch. (high): All .github/ file modifications should be treated as high-risk. Verify recursion guards, self-filter logic, cascade prevention, branch-ignore filters, and null checks on trigger fields.
